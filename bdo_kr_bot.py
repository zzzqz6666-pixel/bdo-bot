...
def resolve_db_path() -> str:
    ...
    if env_path:
        path = Path(env_path)
    elif os.getenv("RAILWAY_MODE", "false").lower() == "true" and Path("/data").exists():
        path = Path("/data") / "bdo_fast_record.db"
    else:
        path = Path("bdo_fast_record.db")
...
SCAN_LOOP_SECONDS = int(os.getenv("SCAN_LOOP_SECONDS", "5"))
...
PENDING_CANDIDATE_ALERTS = os.getenv("PENDING_CANDIDATE_ALERTS", "false").lower() == "true"
PENDING_CONFIRM_BATCH_LIMIT = int(os.getenv("PENDING_CONFIRM_BATCH_LIMIT", "2"))
PENDING_RECHECK_DELAY_MINUTES = int(os.getenv("PENDING_RECHECK_DELAY_MINUTES", "5"))
PENDING_CONFIRM_INTERVAL_SECONDS = int(os.getenv("PENDING_CONFIRM_INTERVAL_SECONDS", "30"))
...
OVERDUE_SCAN_MULTIPLIER = float(os.getenv("OVERDUE_SCAN_MULTIPLIER", "1.2"))
CATCHUP_SCAN_BATCH_LIMIT = int(os.getenv("CATCHUP_SCAN_BATCH_LIMIT", "30"))
FORCE_SCAN_RECORDS_EVENTS = os.getenv("FORCE_SCAN_RECORDS_EVENTS", "true").lower() == "true"
...
BROWSER_COOLDOWN_SECONDS = float(os.getenv("BROWSER_COOLDOWN_SECONDS", "0.15"))
...
def is_wednesday_kst() -> bool:
    return kst_now_dt().weekday() == 2
...
class Database:
    ...
    def _init_db(self):
        ...
        CREATE TABLE IF NOT EXISTS guild_rename_alerts (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            old_guild_name TEXT NOT NULL,
            new_guild_name TEXT NOT NULL,
            ...
            ratio REAL NOT NULL,
            detected_at TEXT NOT NULL,
            UNIQUE(old_guild_name, new_guild_name, alert_date)
        );

        CREATE INDEX IF NOT EXISTS idx_guild_rename_alerts_date
        ON guild_rename_alerts(alert_date);

        CREATE TABLE IF NOT EXISTS parse_fingerprints (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            guild_name TEXT NOT NULL,
            member_count INTEGER NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_parse_fingerprints_guild
        ON parse_fingerprints(guild_name, detected_at DESC);
        ...
        CREATE TABLE IF NOT EXISTS pending_alerts (
            ...
            alert_date TEXT NOT NULL,
            detected_at TEXT NOT NULL,
            UNIQUE(guild_name, family_name, event_type, alert_date)
        );

        CREATE INDEX IF NOT EXISTS idx_pending_alerts_date
        ON pending_alerts(alert_date);
    ...
    def has_guild_rename_alert(self, old_guild_name: str, new_guild_name: str, alert_date: str) -> bool:
        cur = self.conn.cursor()
        cur.execute("""
            SELECT 1
            FROM guild_rename_alerts
            WHERE lower(old_guild_name) = lower(?)
              AND lower(new_guild_name) = lower(?)
              AND alert_date = ?
            LIMIT 1
        """, (old_guild_name, new_guild_name, alert_date))
        return cur.fetchone() is not None

    def record_guild_rename_alert(self, old_guild_name: str, new_guild_name: str, alert_date: str,
                                  overlap_count: int, old_count: int, new_count: int, ratio: float):
        cur = self.conn.cursor()
        cur.execute("""
            INSERT OR IGNORE INTO guild_rename_alerts (
                old_guild_name, new_guild_name, alert_date,
                overlap_count, old_count, new_count, ratio, detected_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            old_guild_name, new_guild_name, alert_date,
            overlap_count, old_count, new_count, ratio, utc_now_text()
        ))
        self.conn.commit()
        return cur.rowcount
    ...
    def record_parse_fingerprint(self, guild_name: str, member_count: int, fingerprint: str):
        cur = self.conn.cursor()
        cur.execute("""
            INSERT INTO parse_fingerprints (guild_name, member_count, fingerprint, detected_at)
            VALUES (?, ?, ?, ?)
        """, (guild_name, member_count, fingerprint, utc_now_text()))
        self.conn.commit()

    def get_recent_parse_fingerprints(self, guild_name: str, limit: int = 5):
        cur = self.conn.cursor()
        cur.execute("""
            SELECT member_count, fingerprint, detected_at
    ...
    def record_pending_alert(self, guild_name: str, family_name: str, event_type: str, alert_date: str) -> int:
        cur = self.conn.cursor()
        cur.execute("""
            INSERT OR IGNORE INTO pending_alerts (
                guild_name, family_name, event_type, alert_date, detected_at
            )
            VALUES (?, ?, ?, ?, ?)
        """, (guild_name, family_name, event_type, alert_date, utc_now_text()))
        self.conn.commit()
        return cur.rowcount
    ...
    def get_due_guilds(self, limit: int):
        ...
        cur.execute("""
            ...
            ORDER BY
                ...
                priority DESC,
                next_scan_after ASC,
                guild_name COLLATE NOCASE
            LIMIT ?
        """, (now, limit))
        return cur.fetchall()

    def get_overdue_guilds(self, limit: int):
        now = utc_now()
        cur = self.conn.cursor()
        cur.execute("""
            SELECT guild_name, priority, scan_interval_min, last_success_at, next_scan_after
            FROM watched_guilds
            WHERE enabled = 1
              AND last_success_at IS NOT NULL
            ORDER BY last_success_at ASC, priority DESC, guild_name COLLATE NOCASE
        """)
        rows = []
        for r in cur.fetchall():
            try:
                last = parse_iso(r["last_success_at"])
                interval = int(r["scan_interval_min"] or DEFAULT_SCAN_INTERVAL_MIN)
                overdue_after = last + timedelta(minutes=max(1, int(interval * OVERDUE_SCAN_MULTIPLIER)))
                if overdue_after <= now:
                    rows.append(r)
            except Exception:
                continue
            if len(rows) >= limit:
    ...
    def has_pending_event(self, guild_name: str, family_name: str, event_type: str) -> bool:
        cur = self.conn.cursor()
        cur.execute("""
            SELECT 1
            FROM pending_events
    ...
    def get_due_pending_events(self, limit: int = 2):
        cur = self.conn.cursor()
        cur.execute("""
    ...
    def reschedule_pending_by_id(self, pending_id: int, minutes: int = 5):
        next_time = (utc_now() + timedelta(minutes=minutes)).isoformat(timespec="seconds")
        cur = self.conn.cursor()
        cur.execute("""
            UPDATE pending_events
            SET confirm_after = ?, last_seen_at = ?
            WHERE id = ?
        """, (next_time, utc_now_text(), pending_id))
        self.conn.commit()
...
async def send_ephemeral_safe(interaction: discord.Interaction, content: str = None, *, embed: discord.Embed = None):
    try:
...
def member_set_fingerprint(members: Set[str]) -> str:
    raw = "\n".join(sorted(members))
    return hashlib.sha1(raw.encode("utf-8", errors="ignore")).hexdigest()[:12]


def parse_health_check(html: str, members: Set[str]) -> tuple[bool, str]:
    if not html or len(html) < 1000:
        return False, "HTML 너무 짧음"

    bad_keywords = ("Incapsula", "Request unsuccessful", "incident ID", "NOINDEX, NOFOLLOW")
    if any(k in html for k in bad_keywords):
        return False, "차단/오류 페이지 감지"

    if "Adventure/Profile" not in html and not members:
        return False, "프로필 링크 없음"

    if len(members) < MIN_VALID_MEMBER_COUNT:
        return False, f"파싱 인원 부족: {len(members)}명"
...
_browser = None
_context = None
_browser_semaphore = asyncio.Semaphore(BROWSER_CONCURRENCY)
...
async def fetch_guild_html_browser(guild_name: str) -> tuple[str, str]:
    url = build_guild_url(guild_name)

    async with _browser_semaphore:
        # 짧은 분산 대기: 250개 대량 스캔 시 순번 밀림 최소화
        ...
        try:
            ...
            page.set_default_timeout(BROWSER_WAIT_SECONDS * 1000)
            page.set_default_navigation_timeout(BROWSER_PAGE_TIMEOUT * 1000)
            ...
            await asyncio.sleep(max(BROWSER_COOLDOWN_SECONDS, 0.2))
            html = await page.content()
...
async def fetch_guild_members(guild_name: str) -> GuildScanResult:
    html, url = await fetch_guild_html_browser(guild_name)
    members = extract_members_from_guild_html(html)

    ok, reason = parse_health_check(html, members)
    if not ok:
        try:
            debug_dir = Path("debug_pages")
            debug_dir.mkdir(exist_ok=True)
            safe_name = re.sub(r"[^0-9A-Za-z가-힣_-]+", "_", guild_name)
            (debug_dir / f"debug_parse_fail_{safe_name}.html").write_text(html[:700000], encoding="utf-8", errors="ignore")
        except Exception:
            pass
        raise RuntimeError(f"파싱 검증 실패: {reason}")

    log.info(
        "파싱 완료 | 길드=%s | 인원=%s | fingerprint=%s",
        guild_name,
        len(members),
        member_set_fingerprint(members),
    )
...
async def scan_one_guild(guild_name: str, interval_min: int, confirm_immediately: bool = False):
    ...
    current_fingerprint = member_set_fingerprint(result.members)
    try:
        db.record_parse_fingerprint(guild_name, len(result.members), current_fingerprint)
        recent_fp = db.get_recent_parse_fingerprints(guild_name, 4)
        if len(recent_fp) >= 4 and len({r["fingerprint"] for r in recent_fp}) == 1:
            log.info("동일 파싱결과 반복 | 길드=%s | 인원=%s | fingerprint=%s", guild_name, len(result.members), current_fingerprint)
    except Exception as fp_error:
        log.exception("파싱 fingerprint 기록 실패 | 길드=%s | %s", guild_name, fp_error)
    ...
    if not is_first_scan:
        ...
        if confirm_immediately:
            # 강제스캔은 현재 변화 확인 목적이므로 가입/탈퇴를 즉시 기록한다.
            immediate_joined = [
                x for x in raw_joined
                if db.get_latest_event_type(guild_name, x) != "join"
            ]
            immediate_left = [
                x for x in raw_left
                if db.get_latest_event_type(guild_name, x) != "leave"
            ]

            if immediate_joined or immediate_left:
                db.insert_events(guild_name, immediate_joined, immediate_left, result.source_url)

            for family_name in immediate_joined:
                db.delete_pending_event(guild_name, family_name, "join")
            for family_name in immediate_left:
                db.delete_pending_event(guild_name, family_name, "leave")

            effective_joined = immediate_joined
            effective_left = immediate_left
        else:
            # 자동 스캔은 오탐 방지를 위해 pending 등록 후 재확인한다.
            confirmed_joined = []
            confirmed_left = []

            for family_name in raw_joined:
                if db.has_pending_event(guild_name, family_name, "join"):
                    confirmed_joined.append(family_name)
                    db.delete_pending_event(guild_name, family_name, "join")
                elif db.get_latest_event_type(guild_name, family_name) != "join":
                    db.upsert_pending_event(guild_name, family_name, "join", result.source_url)

            for family_name in raw_left:
                if db.has_pending_event(guild_name, family_name, "leave"):
                    confirmed_left.append(family_name)
                    db.delete_pending_event(guild_name, family_name, "leave")
                elif db.get_latest_event_type(guild_name, family_name) != "leave":
            ...
            if confirmed_joined or confirmed_left:
                ...
                effective_joined = confirmed_joined
                effective_left = confirmed_left

                log.info(
                    "pending 연속감지 즉시확정 | 길드=%s | 가입=%s | 탈퇴=%s",
                    guild_name,
                    len(confirmed_joined),
                    len(confirmed_left),
                )
    ...
    guild_rename_suspicions = detect_guild_rename_similarity(guild_name, result.members)
    ...
    return {
        ...
        "guild_rename_suspicions": guild_rename_suspicions,
...
async def on_ready():
    ...
    log.info("DB_PATH 사용 중: %s", DB_PATH)
...
def detect_guild_rename_similarity(new_guild_name: str, new_members: Set[str]):
    # 길드명 변경은 수요일 점검 이후에만 추정 알림
    if not is_wednesday_kst():
        return []

    if not new_members or len(new_members) < max(5, MIN_VALID_MEMBER_COUNT):
        return []

    results = []
    new_set = set(new_members)
    new_key = normalize_name(new_guild_name).lower()

    for row in db.list_watched_guilds():
        old_guild_name = row["guild_name"]
        if normalize_name(old_guild_name).lower() == new_key:
            continue

        old_members = set(db.get_snapshot_members_all(old_guild_name))
        if len(old_members) < 5:
            continue

        overlap = sorted(old_members & new_set)
        overlap_count = len(overlap)
        old_count = len(old_members)
        new_count = len(new_set)

        old_ratio = overlap_count / max(old_count, 1)
        new_ratio = overlap_count / max(new_count, 1)
        ratio = min(old_ratio, new_ratio)

        if ratio < 0.5 or overlap_count < 5:
            continue

        confidence = "높음" if ratio >= 0.7 else "중간"
        results.append({
            "old_guild_name": old_guild_name,
    ...
    results.sort(key=lambda x: (x["ratio"], x["overlap_count"]), reverse=True)
    return results[:3]


async def notify_guild_rename_similarity(summary: dict):
    candidates = summary.get("guild_rename_suspicions") or []
    if not candidates:
        return

    alert_date = kst_now_dt().strftime("%Y-%m-%d")

    for candidate in candidates:
        old_guild = candidate["old_guild_name"]
        new_guild = candidate["new_guild_name"]

        # 같은 날 같은 후보 중복 알림 방지
        inserted = db.record_guild_rename_alert(
            old_guild,
            new_guild,
            alert_date,
            int(candidate["overlap_count"]),
            int(candidate["old_count"]),
            int(candidate["new_count"]),
        ...
        for discord_guild in bot.guilds:
            channel = discord.utils.get(discord_guild.text_channels, name=ADMIN_CHANNEL_NAME)
            if channel is None:
                continue

            percent = int(candidate["ratio"] * 100)
            ...
            embed = make_embed(
                ...
                description=(
                    f"기존 길드: **{old_guild}**\n"
                    f"새 길드 후보: **{new_guild}**\n"
                    f"겹치는 구성원: **{candidate['overlap_count']}명**\n"
                    f"유사도: **{percent}%** / 추정도 **{candidate['confidence']}**\n"
                    f"감지일: {alert_date} KST"
                ),
                color=0x9B59B6,
            )
            embed.add_field(
                name="구성원 수 비교",
                value=f"기존 {candidate['old_count']}명 / 후보 {candidate['new_count']}명",
                inline=False
            )
            embed.add_field(
                name="겹치는 가문 샘플",
                value=limit_text([f"`{x}`" for x in sample], 1000) if sample else "-",
                inline=False
            )
            embed.add_field(
                name="권장 처리",
                value="새 길드명으로 1~2회 스캔 성공 확인 후 기존 길드명을 제거하세요.",
                inline=False
            )
...
async def notify_pending_candidate_changes(summary: dict):
    return
...
async def notify_changes(summary: dict):
    for discord_guild in bot.guilds:
        ...
        guild_rename_suspicions = summary.get("guild_rename_suspicions") or []
        ...
        if guild_rename_suspicions:
            lines = []
            for x in guild_rename_suspicions[:3]:
                percent = int(float(x.get("ratio", 0)) * 100)
                lines.append(
                    f"🟣 `{x['old_guild_name']}` → `{x['new_guild_name']}` | "
                    f"겹침 {x['overlap_count']}명 / 유사도 {percent}% / {x['confidence']}"
                )
            embed.add_field(
                name="🟣 길드명 변경 추정",
                value=limit_text(lines, 1000),
                inline=False
            )

        await notify_guild_rename_similarity(summary)
...
async def confirm_pending_events():
    global _last_pending_confirm_at

    # pending 재확인은 별도 소량 큐처럼 처리한다.
    # 후보가 수십 개 생겨도 공홈 요청이 폭주하지 않게 최소 간격을 둔다.
    now = utc_now()
    if _last_pending_confirm_at is not None:
        elapsed = (now - _last_pending_confirm_at).total_seconds()
        if elapsed < PENDING_CONFIRM_INTERVAL_SECONDS:
            return
    _last_pending_confirm_at = now

    db.cleanup_expired_pending_events()
    rows = db.get_due_pending_events(PENDING_CONFIRM_BATCH_LIMIT)
    if not rows:
        return
    ...
    for guild_name, pendings in grouped.items():
        ...
        try:
            result = await fetch_guild_members(guild_name)
            if len(result.members) < MIN_VALID_MEMBER_COUNT:
                db.mark_scan_failure(guild_name, "pending 확인 중 구성원 0명")
                log.warning("pending 확인 스킵 | 길드=%s | 구성원 0명", guild_name)
                for p in pendings:
                    db.reschedule_pending_by_id(int(p["id"]), PENDING_RECHECK_DELAY_MINUTES)
                continue
            ...
            for p in pendings:
                ...
                if event_type == "join":
                    ...
                    elif family_name in result.members:
                        db.delete_pending_by_id(pid)
                        cancelled += 1
                    else:
                        db.reschedule_pending_by_id(pid, PENDING_RECHECK_DELAY_MINUTES)
                        rescheduled += 1
                    ...
                    if family_name not in result.members and db.get_latest_event_type(guild_name, family_name) != "leave":
                        confirmed_left.append(family_name)
                        db.delete_pending_by_id(pid)
                    elif family_name not in result.members:
                        db.delete_pending_by_id(pid)
                        cancelled += 1
                    else:
                        db.reschedule_pending_by_id(pid, PENDING_RECHECK_DELAY_MINUTES)
            ...
            if confirmed_joined or confirmed_left:
                ...
                summary = {
                    ...
                    "left": confirmed_left,
                    "rename_suspicions": rename_suspicions,
                    "guild_rename_suspicions": detect_guild_rename_similarity(guild_name, result.members) if "detect_guild_rename_similarity" in globals() else [],
                    "pending_joined": [],
                    "pending_left": [],
                    "fetched_at": result.fetched_at,
                    "first_scan": False,
            ...
            if rescheduled:
                log.info("pending 재확인 예약 | 길드=%s | %s건", guild_name, rescheduled)
            if cancelled:
                log.info("pending 중복/이미처리 정리 | 길드=%s | %s건", guild_name, cancelled)
...
async def scheduler_loop():
    ...
    batch_limit = max(MAX_CONCURRENT_REQUESTS, CATCHUP_SCAN_BATCH_LIMIT)
    overdue_rows = db.get_overdue_guilds(batch_limit) if hasattr(db, "get_overdue_guilds") else []
    due_rows = db.get_due_guilds(batch_limit)

    seen = set()
    rows = []
    for r in list(overdue_rows) + list(due_rows):
        key = str(r["guild_name"]).lower()
        if key in seen:
            continue
        seen.add(key)
        rows.append(r)
    ...
    jobs = [scan_one_guild(r["guild_name"], int(r["scan_interval_min"]), confirm_immediately=False) for r in rows]
    ...
    for row, res in zip(rows, results):
        ...
        if isinstance(res, Exception):
            ...
            continue
        log.info("스캔 완료 | 길드=%s 인원=%s 확정가입=%s 확정탈퇴=%s 후보가입=%s 후보탈퇴=%s", guild_name, res["member_count"], len(res["joined"]), len(res["left"]), len(res.get("pending_joined", [])), len(res.get("pending_left", [])))
        if res["joined"] or res["left"] or res["rename_suspicions"] or res.get("guild_rename_suspicions"):
...
_first_scan_queue: Optional[asyncio.Queue] = None
_first_scan_worker_started = False


async def _first_scan_worker():
    global _first_scan_queue

    if _first_scan_queue is None:
        _first_scan_queue = asyncio.Queue()

    while True:
        guild_name, chosen = await _first_scan_queue.get()
        try:
            # Playwright/파싱이 명령어 응답을 막지 않도록 별도 스레드에서 새 이벤트루프로 실행
            res = await scan_one_guild(guild_name, chosen)
            log.info(
                "백그라운드 첫 스캔 완료 | 길드=%s | 인원=%s",
                guild_name,
                res.get("member_count"),
            )
        except Exception as e:
            log.exception("백그라운드 첫 스캔 실패 | 길드=%s | %s", guild_name, e)
        finally:
            _first_scan_queue.task_done()
            await asyncio.sleep(0.2)


def enqueue_first_scan(guild_name: str, chosen: int):
    global _first_scan_queue, _first_scan_worker_started
    ...
    if not _first_scan_worker_started:
        loop.create_task(_first_scan_worker())
        _first_scan_worker_started = True

    _first_scan_queue.put_nowait((guild_name, chosen))


def watched_guild_exists(guild_name: str) -> bool:
    try:
        target = normalize_name(guild_name).lower()
        for r in db.list_watched_guilds():
            if normalize_name(r["guild_name"]).lower() == target:
                return True
    except Exception:
        pass
    return False
...
@bot.tree.command(name="길드추가", description="감시 길드를 추가하고 즉시 등록합니다. 첫 스캔은 백그라운드로 진행됩니다.")
@admin_only()
@app_commands.describe(guild_name="길드명", priority="우선 스캔 여부", interval_min="스캔 간격(분)")
async def add_guild(interaction: discord.Interaction, guild_name: str, priority: bool = False, interval_min: Optional[int] = None):
    ...
    if watched_guild_exists(guild_name):
        embed = make_embed("ℹ️ 이미 등록된 길드", color=0x3498DB)
        embed.add_field(name="길드명", value=f"`{guild_name}`", inline=True)
        embed.add_field(name="처리", value="이미 감시 목록에 있어서 다시 추가하지 않았습니다.", inline=False)
        await send_ephemeral_safe(interaction, embed=embed)
        return
    ...
    try:
        ...
        embed = make_embed("✅ 길드 추가 완료", color=0x2ECC71)
        embed.add_field(name="길드명", value=f"`{guild_name}`", inline=True)
        embed.add_field(name="스캔 간격", value=f"{chosen}분", inline=True)
        embed.add_field(name="처리 방식", value="등록은 즉시 완료되었습니다.\n첫 스캔은 대기열에서 순차 진행됩니다.", inline=False)

        await interaction.response.send_message(embed=embed, ephemeral=True)

        enqueue_first_scan(guild_name, chosen)
        ...
        try:
            if interaction.response.is_done():
                await interaction.followup.send(f"❌ 길드 추가 실패: {str(e)[:300]}", ephemeral=True)
            else:
                await interaction.response.send_message(f"❌ 길드 추가 실패: {str(e)[:300]}", ephemeral=True)
        except Exception:
            pass
...
@bot.tree.command(name="길드목록", description="등록된 감시 길드 목록을 관리자 채널에서 봅니다.")
@admin_only()
async def list_guilds(interaction: discord.Interaction):
    ...
    for idx, r in enumerate(rows, start=1):
        ...
        lines.append(f"{idx}. {star} `{r['guild_name']}` | {r['scan_interval_min']}분 | {last}")

    # Discord embed 전체 제한 6000자 회피: 900자 필드 5개씩 여러 embed로 분할
    chunks = chunk_lines_for_embed(lines, max_chars=900, max_chunks=100)
    pages = [chunks[i:i + 5] for i in range(0, len(chunks), 5)]

    for page_no, page_chunks in enumerate(pages, start=1):
        embed = make_embed("📋 감시 길드 목록", color=0x95A5A6)
        embed.description = f"총 **{len(rows)}개** / 페이지 **{page_no}/{len(pages)}**"

        for i, chunk in enumerate(page_chunks, start=1):
            field_no = (page_no - 1) * 5 + i
            embed.add_field(
                name=f"목록 {field_no}",
                value=chunk or "-",
                inline=False
...
async def force_scan(interaction: discord.Interaction, guild_name: str):
    ...
    try:
        res = await scan_one_guild(info["guild_name"], int(info["scan_interval_min"]), confirm_immediately=FORCE_SCAN_RECORDS_EVENTS)
        embed = make_embed(f"🔄 강제 스캔 · {res['guild_name']}", "강제스캔은 감지된 가입/탈퇴를 즉시 기록합니다.", color=0x1ABC9C)
        ...
        if res.get("pending_joined") or res.get("pending_left"):
            embed.add_field(
                name="감지된 변화",
                value=(
                    f"가입 후보 {len(res.get('pending_joined', []))}명 / "
                    f"탈퇴 후보 {len(res.get('pending_left', []))}명"
                ),
                inline=False
            )
        await interaction.followup.send(embed=embed, ephemeral=True)
...
@bot.tree.command(name="db상태", description="DB 저장 경로와 파일 상태를 확인합니다.")
@admin_only()
async def db_status(interaction: discord.Interaction):
    ...
    volume_ok = str(path).startswith("/data")

    embed = make_embed("🗄️ DB 상태", color=0x3498DB)
    embed.add_field(name="DB_PATH", value=f"`{DB_PATH}`", inline=False)
    embed.add_field(name="파일 존재", value="예" if exists else "아니오", inline=True)
    embed.add_field(name="크기", value=f"{size:,} bytes", inline=True)
    embed.add_field(name="Volume 사용", value="예" if volume_ok else "아니오", inline=True)
    await interaction.response.send_message(embed=embed, ephemeral=True)




@bot.tree.command(name="파싱진단", description="특정 길드의 현재 파싱 결과와 직전 스냅샷 비교를 확인합니다.")
@admin_only()
@app_commands.describe(guild_name="길드명")
async def parse_diagnose(interaction: discord.Interaction, guild_name: str):
    if not await require_admin_channel(interaction):
        return
    ...
    try:
        ...
        embed = make_embed("🔎 파싱 진단", color=0x3498DB)
        embed.add_field(name="길드명", value=f"`{guild_name}`", inline=True)
        embed.add_field(name="현재 파싱 인원", value=f"{len(result.members)}명", inline=True)
        embed.add_field(name="직전 스냅샷 인원", value=f"{len(previous)}명", inline=True)
        embed.add_field(name="fingerprint", value=f"`{member_set_fingerprint(result.members)}`", inline=False)
        embed.add_field(name="가입 후보", value=limit_text([f"🟢 `{x}`" for x in joined[:20]], 1000) if joined else "-", inline=False)
        embed.add_field(name="탈퇴 후보", value=limit_text([f"🔴 `{x}`" for x in left[:20]], 1000) if left else "-", inline=False)
        embed.add_field(name="샘플", value=limit_text([f"• `{x}`" for x in sorted(result.members)[:20]], 1000), inline=False)
        await interaction.followup.send(embed=embed, ephemeral=True)
    except Exception as e:
        await interaction.followup.send(f"파싱진단 실패: {e}", ephemeral=True)


@bot.tree.command(name="추적진단", description="최근 스캔/후보/이벤트 상태를 확인합니다.")
@admin_only()
async def tracking_diagnose(interaction: discord.Interaction):
    if not await require_admin_channel(interaction):
        return

    rows = db.list_watched_guilds()
    total = len(rows)
    unscanned = [r for r in rows if not r["last_success_at"]]
    errored = [r for r in rows if r["last_error"]]
    ...
    for r in rows:
        try:
            if r["last_success_at"]:
                mins = (now - parse_iso(r["last_success_at"])).total_seconds() / 60
                if mins >= 60:
                    stale.append((r["guild_name"], int(mins)))
        except Exception:
            pass

    try:
        pending_rows = db.list_pending_events(100)
    except Exception:
        pending_rows = []

    embed = make_embed("🧭 추적 진단", color=0x3498DB)
    embed.add_field(name="등록 길드", value=f"{total}개", inline=True)
    embed.add_field(name="미스캔", value=f"{len(unscanned)}개", inline=True)
    embed.add_field(name="오류 있음", value=f"{len(errored)}개", inline=True)
    embed.add_field(name="60분 이상 미갱신", value=f"{len(stale)}개", inline=True)
    embed.add_field(name="pending 후보", value=f"{len(pending_rows)}개", inline=True)

    if stale:
        lines = [f"• `{g}` | {m}분 전" for g, m in sorted(stale, key=lambda x: x[1], reverse=True)[:10]]
        embed.add_field(name="오래된 스캔 상위", value=limit_text(lines, 1000), inline=False)

    if pending_rows:
        lines = []
        for r in pending_rows[:10]:
            emoji = "🟢" if r["event_type"] == "join" else "🔴"
            lines.append(f"{emoji} `{r['guild_name']}` / `{r['family_name']}` | {kst_display(r['first_detected_at'], short=True)}")
        embed.add_field(name="pending 후보 샘플", value=limit_text(lines, 1000), inline=False)
