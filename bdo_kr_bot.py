...
async def force_scan(interaction: discord.Interaction, guild_name: str):
    ...
    try:
        res = await scan_one_guild(
            info["guild_name"],
            int(info["scan_interval_min"]),
            confirm_immediately=FORCE_SCAN_RECORDS_EVENTS
        )

        embed = make_embed(
            f"🔄 강제 스캔 · {res['guild_name']}",
            "강제스캔은 새로 감지된 가입/탈퇴를 즉시 기록합니다.",
            color=0x1ABC9C
        )
        embed.add_field(name="현재 인원", value=f"**{res['member_count']}명**", inline=True)

        joined = res.get("joined") or []
        left = res.get("left") or []
        raw_joined = res.get("raw_joined") or []
        raw_left = res.get("raw_left") or []

        embed.add_field(
            name="이번 스캔에서 새로 확정된 가입",
            value=limit_text([f"🟢 `{x}`" for x in joined]) if joined else "-",
            inline=False
        )
        embed.add_field(
            name="이번 스캔에서 새로 확정된 탈퇴",
            value=limit_text([f"🔴 `{x}`" for x in left]) if left else "-",
            inline=False
        )

        if raw_joined or raw_left:
            embed.add_field(
                name="이번 비교에서 감지된 원본 변화",
                value=(
                    f"가입 후보 {len(raw_joined)}명 / 탈퇴 후보 {len(raw_left)}명\n"
                    f"{limit_text([f'🟢 `{x}`' for x in raw_joined[:10]] + [f'🔴 `{x}`' for x in raw_left[:10]], 900)}"
                ),
                inline=False
            )
        else:
            embed.add_field(
                name="이번 비교 결과",
                value="직전 스냅샷 대비 새 변화 없음",
                inline=False
            )

        recent = db.get_guild_recent_events(info["guild_name"], 10)
        if recent:
            lines = []
            for r in recent[:10]:
                emoji = "🟢" if r["event_type"] == "join" else "🔴"
