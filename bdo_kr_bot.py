...
PENDING_CANDIDATE_ALERTS = os.getenv("PENDING_CANDIDATE_ALERTS", "true").lower() == "true"
PENDING_CONFIRM_BATCH_LIMIT = int(os.getenv("PENDING_CONFIRM_BATCH_LIMIT", "100"))
...
class Database:
    ...
    def get_due_pending_events(self, limit: int = 100):
        cur = self.conn.cursor()
        cur.execute("""
...
async def notify_pending_candidate_changes(summary: dict):
    if not PENDING_CANDIDATE_ALERTS:
        return
    if summary.get("first_scan"):
        return

    pending_joined = summary.get("pending_joined") or []
    pending_left = summary.get("pending_left") or []
    if not pending_joined and not pending_left:
        return
    ...
    for family_name in pending_joined:
        if db.record_pending_alert(summary["guild_name"], family_name, "join", alert_date):
            new_joined.append(family_name)

    for family_name in pending_left:
        if db.record_pending_alert(summary["guild_name"], family_name, "leave", alert_date):
            new_left.append(family_name)

    if not new_joined and not new_left:
        return

    embed = make_embed(
        title=f"⏳ 가입/탈퇴 후보 감지 · {summary['guild_name']}",
        description=(
            f"현재 인원: **{summary['member_count']}명**\n"
            f"감지 시각: {kst_display(summary['fetched_at'])}\n"
            f"{PENDING_CONFIRM_MINUTES}분 후 재확인되면 확정 알림/연혁에 기록됩니다."
        ),
        color=0xF1C40F,
    )
    ...
    if new_joined:
        embed.add_field(
            name=f"가입 후보 {len(new_joined)}명",
            value=limit_text([f"🟢 `{x}`" for x in new_joined[:20]], 1000),
            inline=False
        )

    if new_left:
        embed.add_field(
            name=f"탈퇴 후보 {len(new_left)}명",
            value=limit_text([f"🔴 `{x}`" for x in new_left[:20]], 1000),
            inline=False
        )

    embed.add_field(
        name="상태",
        value="아직 확정 기록은 아닙니다. 재확인 후에도 유지되면 확정됩니다.",
        inline=False
    )
...
async def notify_changes(summary: dict):
    ...
    try:
        await notify_pending_candidate_changes(summary)
    except Exception as e:
        log.exception("pending 후보 알림 실패 | %s", e)
...
async def confirm_pending_events():
    ...
    for guild_name, pendings in grouped.items():
        ...
        try:
            ...
            for p in pendings:
                ...
                if event_type == "join":
                    if family_name in result.members and db.get_latest_event_type(guild_name, family_name) != "join":
                        ...
                        # 일시 파싱/짧은 변동 가능성 때문에 바로 삭제하지 않고 재확인
                    ...
                    if family_name not in result.members and db.get_latest_event_type(guild_name, family_name) != "leave":
                        ...
                        # 재등장했더라도 한 번은 더 확인 후 후보 만료에 맡김
                        db.reschedule_pending_by_id(pid, PENDING_RECHECK_DELAY_MINUTES)
                        rescheduled += 1

            # snapshot은 pending 판단 후 저장
            db.create_snapshot(guild_name, result.members, result.source_url)
            db.mark_scan_success(guild_name, interval_min)
            ...
            # 실패한 pending은 삭제하지 않고 다음 루프에서 재확인
            for p in pendings:
                try:
                    db.reschedule_pending_by_id(int(p["id"]), PENDING_RECHECK_DELAY_MINUTES)
                except Exception:
...
async def scheduler_loop():
    ...
    for row, res in zip(rows, results):
        ...
        if res["joined"] or res["left"] or res["rename_suspicions"] or res.get("guild_rename_suspicions") or res.get("pending_joined") or res.get("pending_left"):
